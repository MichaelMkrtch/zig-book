...
<Callout title="Note: Constants and Imports">
  ...
  imports are compile-time operations. Therefore, the result of an import, which
  is needed at compile-time, must be assigned to a constant so it's evaluated at
  compile-time.
</Callout>
